...
public class MyNicePlayerController extends NiceVideoPlayerController implements OnClickListener, OnSeekBarChangeListener, OnClarityChangedListener {
    ...
    private BroadcastReceiver mBatterReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (status == 2) {
                MyNicePlayerController.this.mBattery.setImageResource(drawable.battery_charging);
            } else if (status == 5) {
                MyNicePlayerController.this.mBattery.setImageResource(drawable.battery_full);
            } else {
                int level = intent.getIntExtra("level", 0);
                int scale = intent.getIntExtra("scale", 0);
                int percentage = (int)((float)level / (float)scale * 100.0F);
                if (percentage <= 10) {
                    MyNicePlayerController.this.mBattery.setImageResource(drawable.battery_10);
                } else if (percentage <= 20) {
                    MyNicePlayerController.this.mBattery.setImageResource(drawable.battery_20);
                } else if (percentage <= 50) {
                    MyNicePlayerController.this.mBattery.setImageResource(drawable.battery_50);
                } else if (percentage <= 80) {
                    MyNicePlayerController.this.mBattery.setImageResource(drawable.battery_80);
                } else if (percentage <= 100) {
                    MyNicePlayerController.this.mBattery.setImageResource(drawable.battery_100);
                }
            }
    ...
    public MyNicePlayerController(Context context) {
        super(context);
    ...
    private void init() {
        LayoutInflater.from(this.mContext).inflate(R.layout.my_nice_player_controller, this, true);
        ...
        if (this.mDismissTopBottomCountDownTimer == null) {
            this.mDismissTopBottomCountDownTimer = new CountDownTimer(8000L, 8000L) {
                ...
                public void onFinish() {
                    MyNicePlayerController.this.setTopBottomVisible(false);
                }
            };
        }
